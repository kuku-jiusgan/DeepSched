...
def project_hours_report(
    ...
    status: str | None = Query(None, max_length=100, description="项目状态，逗号分隔"),
    token: str = Depends(auth_token), db: Session = Depends(get_db),
):
    return build_project_hours_report(
        db, get_current_user(token, db), start_date, end_date, keyword,
        parse_project_statuses(status),
    )
