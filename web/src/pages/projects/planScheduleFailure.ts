...
export function scheduleFailureContent(rawMessage: string) {
  ...
  const projectSource = checklistText ? summary : rawMessage
  ...
  while ((match = projectPattern.exec(projectSource)) !== null) {
  ...
  return h('div', { class: 'schedule-failure-content' }, [
    h('div', { class: 'schedule-failure-summary' }, projectSource.split('。')[0]),
    ...
    checklistText ? h('div', { class: 'schedule-failure-section-title' }, '请按顺序检查') : null,
    checklistText ? h('ol', { class: 'schedule-failure-checks' }, checks.map(check => h('li', check))) : null,
